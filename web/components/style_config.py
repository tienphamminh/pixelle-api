...
def render_style_config(pixelle_video):
    ...
    if template_requires_media:
        ...
        with st.container(border=True):
            ...
            image_motion_preset = "auto"
            subtitle_mode = "scene"
            subtitle_style = "comic_tiktok"
            subtitle_words_per_chunk = 4
            if template_media_type != "video":
                st.markdown("**Image Motion**")
                motion_display = st.selectbox(
                    "Image motion",
                    ["Static", "Ken Burns (zoom / pan)"],
                    index=0,
                    help="Apply camera-like zoom/pan motion to image workflow outputs.",
                    key="image_motion_select"
                )
                image_motion = "ken_burns" if motion_display.startswith("Ken Burns") else "static"

                if image_motion == "ken_burns":
                    strength_display = st.select_slider(
                        "Motion strength",
                        options=["Subtle", "Medium", "Strong"],
                        value="Subtle",
                        help="Subtle is closest to motion-comic / slideshow references.",
                        key="image_motion_strength_select"
                    )
                    image_motion_strength = strength_display.lower()

                    preset_options = {
                        "Auto rotate scenes": "auto",
                        ...
                        "Pan up": "pan_up",
                        "Pan down": "pan_down",
                        "Drift upper left": "drift_upper_left",
                        "Drift upper right": "drift_upper_right",
                        "Drift lower left": "drift_lower_left",
                        "Drift lower right": "drift_lower_right",
                        "Push in left": "push_in_left",
                        "Push in right": "push_in_right",
                        "Push out left": "push_out_left",
                        "Push out right": "push_out_right",
                    }
                    preset_display = st.selectbox(
                        "Motion effect",
                        list(preset_options.keys()),
                        index=0,
                        help="Auto gives each scene a different smooth comic-panel camera move.",
                        key="image_motion_preset_select"
                    )
                    image_motion_preset = preset_options[preset_display]

            st.markdown("**Subtitles**")
            subtitle_options = {
                "Template scene subtitle": "scene",
                "No subtitle": "none",
                "TikTok phrase subtitle": "phrase",
            }
            subtitle_display = st.selectbox(
                "Subtitle mode",
                list(subtitle_options.keys()),
                index=0,
                help="Phrase subtitles are burned into the video as short timed chunks.",
                key="subtitle_mode_select"
            )
            subtitle_mode = subtitle_options[subtitle_display]

            if subtitle_mode == "phrase":
                subtitle_style_options = {
                    "Comic TikTok": "comic_tiktok",
                    "Clean white": "clean_white",
                    "Yellow keyword": "yellow_keyword",
                }
                subtitle_style_display = st.selectbox(
                    "Subtitle style",
                    list(subtitle_style_options.keys()),
                    index=0,
                    key="subtitle_style_select"
                )
                subtitle_style = subtitle_style_options[subtitle_style_display]
                subtitle_words_per_chunk = st.slider(
                    "Words per subtitle chunk",
                    min_value=2,
                    max_value=8,
                    value=4,
                    help="Lower values feel punchier; higher values are calmer and easier to read.",
                    key="subtitle_words_per_chunk_slider"
                )
    ...
    return {
        ...
        "image_motion_strength": image_motion_strength,
