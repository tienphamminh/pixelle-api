...
def render_single_output(pixelle_video, video_params):
    ...
    manual_storyboard = video_params.get("manual_storyboard")
    manual_storyboard_error = video_params.get("manual_storyboard_error")
    ...
    image_motion = video_params.get("image_motion", "static")
    image_motion_strength = video_params.get("image_motion_strength", "subtle")
    image_motion_preset = video_params.get("image_motion_preset", "auto")
    subtitle_mode = video_params.get("subtitle_mode", "scene")
    subtitle_style = video_params.get("subtitle_style", "comic_tiktok")
    subtitle_words_per_chunk = video_params.get("subtitle_words_per_chunk", 4)
        ...
        if st.button(tr("btn.generate"), type="primary", use_container_width=True):
            ...
            if mode == "manual_storyboard":
                if manual_storyboard_error:
                    ...
                    st.stop()
                if not manual_storyboard:
                    st.error(tr("error.manual_storyboard_required"))
                    st.stop()
            elif not text:
            ...
            try:
                ...
                video_params = {
                    ...
                    "manual_storyboard": manual_storyboard if mode == "manual_storyboard" else None,
