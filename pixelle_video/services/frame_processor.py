...
class FrameProcessor:
    ...
    async def _compose_frame_html(
        ...
        subtitle_mode = (config.subtitle_mode or "scene").lower()
        template_text = "" if subtitle_mode in {"none", "phrase"} else frame.narration
    ...
    async def _step_create_video_segment(
        ...
        subtitle_mode = (config.subtitle_mode or "scene").lower()
        use_phrase_subtitles = subtitle_mode == "phrase"
        render_output_path = output_path
        if use_phrase_subtitles:
            stem, ext = os.path.splitext(output_path)
            render_output_path = f"{stem}_base{ext}"
        ...
        if frame.media_type == "video":
            ...
            segment_path = video_service.create_video_from_image(
                ...
                motion=config.image_motion,
                motion_strength=config.image_motion_strength,
                motion_preset=config.image_motion_preset,
        ...
        if use_phrase_subtitles:
            subtitle_path = os.path.join(
                ...
                f"{frame.index + 1:02d}_subtitle.ass"
            )
            width, height = video_service._probe_video_size(segment_path)
            video_service.create_phrase_subtitle_file(
                text=frame.narration,
                output=subtitle_path,
                duration=frame.duration or await self._get_video_duration(segment_path),
                width=width,
                height=height,
                style=config.subtitle_style,
                words_per_chunk=config.subtitle_words_per_chunk,
            )
            segment_path = video_service.burn_subtitles(
                video=segment_path,
                subtitle_path=subtitle_path,
                output=output_path,
            )
            if render_output_path != output_path and os.path.exists(render_output_path):
                os.unlink(render_output_path)
    ...
    async def _download_media(
        ...
        if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
            logger.debug(f"  -> Using existing downloaded media: {output_path}")
            return output_path

        timeout = httpx.Timeout(connect=20.0, read=180.0, write=60.0, pool=60.0)
        temp_output_path = f"{output_path}.download"
        ...
        async with httpx.AsyncClient(
            ...
            for attempt in range(1, max_attempts + 1):
                try:
                    ...
                    logger.debug(
                        f"  -> Downloading {media_type} for frame {frame_index + 1} "
                ...
                if attempt < max_attempts:
                    ...
                    logger.warning(
                        f"Download failed for frame {frame_index + 1} "
                        f"({type(last_error).__name__}); retrying in {delay}s..."
                    )
                    await asyncio.sleep(delay)

        raise RuntimeError(
            f"Failed to download {media_type} for frame {frame_index + 1} "
