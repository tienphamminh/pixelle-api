...
def _normalize_manual_storyboard(value: Any) -> List[Dict[str, str]]:
    """Normalize a manual storyboard list from UI/API input."""
    ...
    if not isinstance(value, list) or not value:
        raise ValueError("manual_storyboard must be a non-empty JSON list")

    storyboard: List[Dict[str, str]] = []
    for idx, item in enumerate(value, start=1):
        if not isinstance(item, dict):
            raise ValueError(f"manual_storyboard scene {idx} must be an object")

        narration = item.get("narration")
        image_prompt = item.get("image_prompt")

        if not isinstance(narration, str) or not narration.strip():
            raise ValueError(f"manual_storyboard scene {idx} is missing narration")
        if not isinstance(image_prompt, str) or not image_prompt.strip():
            raise ValueError(f"manual_storyboard scene {idx} is missing image_prompt")
...
class StandardPipeline(LinearVideoPipeline):
    ...
    - "manual_storyboard": Use user-provided narration + image_prompt pairs as-is
    """
    ...
    async def generate_content(self, ctx: PipelineContext):
        ...
        if mode == "manual_storyboard":
            self._report_progress(ctx.progress_callback, "preparing_frames", 0.05)
            manual_storyboard = _normalize_manual_storyboard(
                ctx.params.get("manual_storyboard") or text
            )
            ctx.params["manual_storyboard"] = manual_storyboard
            ctx.narrations = [scene["narration"] for scene in manual_storyboard]
            logger.info(f"✅ Loaded {len(ctx.narrations)} manual storyboard scenes")
        elif mode == "generate":
    ...
    async def determine_title(self, ctx: PipelineContext):
        ...
        if title:
            ...
            if mode == "generate":
                ...
                title_source = ctx.narrations[0] if ctx.narrations else text
                ctx.title = await generate_title(self.llm, title_source, strategy="direct")
                logger.info(f"   Title: '{ctx.title}' (derived from manual storyboard)")
            else:  # fixed
    ...
    async def plan_visuals(self, ctx: PipelineContext):
        ...
        if mode == "manual_storyboard":
            manual_storyboard = _normalize_manual_storyboard(ctx.params.get("manual_storyboard"))
            if template_requires_media:
                self._report_progress(ctx.progress_callback, "preparing_frames", 0.15)
                ctx.image_prompts = [scene["image_prompt"] for scene in manual_storyboard]
                logger.info(f"✅ Loaded {len(ctx.image_prompts)} manual image prompts")
            else:
                ctx.image_prompts = [None] * len(ctx.narrations)
                logger.info("⚡ Static template - ignored manual image prompts")
            return
    ...
    async def initialize_storyboard(self, ctx: PipelineContext):
        ...
        ctx.config = StoryboardConfig(
            ...
            image_motion=ctx.params.get("image_motion", "static"),
            image_motion_strength=ctx.params.get("image_motion_strength", "subtle"),
            image_motion_preset=ctx.params.get("image_motion_preset", "auto"),
            subtitle_mode=ctx.params.get("subtitle_mode", "scene"),
            subtitle_style=ctx.params.get("subtitle_style", "comic_tiktok"),
            subtitle_words_per_chunk=ctx.params.get("subtitle_words_per_chunk", 4),
            frame_template=ctx.params.get("frame_template") or "1080x1920/default.html",
    ...
    async def post_production(self, ctx: PipelineContext):
        ...
        # Ken Burns segments render at 60fps; join them by re-encoding onto one uniform
        # CFR timeline (concat filter) so scene transitions don't hitch. Static-only
        # output keeps the faster demuxer copy path.
        use_motion = (ctx.params.get("image_motion", "static") or "static").lower() == "ken_burns"
        final_video_path = video_service.concat_videos(
            videos=segment_paths,
            output=ctx.final_video_path,
            method="filter" if use_motion else "demuxer",
            target_fps=max(ctx.params.get("video_fps", 30), 60) if use_motion else None,
