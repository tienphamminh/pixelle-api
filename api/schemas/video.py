...
class VideoGenerateRequest(BaseModel):
    ...
    # === Processing Mode ===
    mode: Literal["generate", "fixed", "manual_storyboard"] = Field(
        "generate",
        description="Processing mode: 'generate' (AI generates narrations), 'fixed' (use text as-is), or 'manual_storyboard' (use provided narration/image_prompt pairs)"
    )

    manual_storyboard: Optional[List[Dict[str, str]]] = Field(
        None,
        description="Manual storyboard scenes for manual_storyboard mode. Each item requires 'narration' and 'image_prompt'."
    )
    ...
    tts_inference_mode: Literal["local", "comfyui"] = Field(
        "local",
        description="TTS inference mode: 'local' for Edge TTS or 'comfyui' for workflow-based TTS."
    )
    tts_voice: Optional[str] = Field(
        None,
        description="TTS voice ID for local synthesis (e.g., 'en-US-JennyNeural')."
    )
    tts_speed: Optional[float] = Field(
        1.0,
        ge=0.5,
        le=2.0,
        description="TTS speed multiplier for local synthesis."
    )
    ...
    image_motion: Literal["static", "ken_burns"] = Field(
        "static",
        description="Motion applied when converting image workflow outputs into video segments."
    )
    image_motion_strength: Literal["subtle", "medium", "strong"] = Field(
        "subtle",
        description="Motion strength for image_motion='ken_burns'."
    )
    image_motion_preset: Literal[
        "auto",
        "slow_zoom_in",
        "slow_zoom_out",
        "pan_left",
        ...
        "auto",
        description="Ken Burns motion preset. 'auto' rotates presets by scene index."
    )
    subtitle_mode: Literal["scene", "none", "phrase"] = Field(
        "scene",
        description="Subtitle rendering mode. 'scene' keeps template text, 'phrase' burns timed TikTok-style chunks."
    )
    subtitle_style: Literal["comic_tiktok", "clean_white", "yellow_keyword"] = Field(
        "comic_tiktok",
        description="Visual style used when subtitle_mode='phrase'."
    )
    subtitle_words_per_chunk: int = Field(
        4,
        ge=2,
        le=8,
        description="Target number of words per timed phrase subtitle chunk."
    )
